...
describe('CandlV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: CandlV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as CandlV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as CandlV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [CandlV3Factory, CandlV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('CandlV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('CandlV3Pool')
