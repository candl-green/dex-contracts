...
import { TestCandlV3SwapPay } from '../typechain-types/contracts/test/TestCandlV3SwapPay'
...
import { TestCandlV3ReentrantCallee } from '../typechain-types/contracts/test/TestCandlV3ReentrantCallee'
...
describe('CandlV3Pool', () => {
  ...
  let factory: CandlV3Factory
  let pool: MockTimeCandlV3Pool

  let swapTarget: TestCandlV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeCandlV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestCandlV3ReentrantCallee')
      ).deploy()) as TestCandlV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestCandlV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestCandlV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestCandlV3SwapPay
