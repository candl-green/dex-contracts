...
/// @title Canonical CandlSwap V3 factory
/// @notice Deploys CandlSwap V3 pools and manages ownership and control over pool protocol fees
contract CandlV3Factory is ICandlV3Factory {
    /// @inheritdoc ICandlV3Factory
    address public override owner;

    address private ignoreAddressForChangeBuildBytes;
    ...
    function createPool(
        ...
        pool = ICandlV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ICandlV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ICandlV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ICandlV3Pool(pool).setLmPool(lmPool);
    }
}
