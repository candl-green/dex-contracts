...
The contract that deployed the pool, which must adhere to the ICandlV3Factory interface
...
```solidity
struct CandlV3Pool.Slot0 slot0
```
...
```solidity
struct CandlV3Pool.ProtocolFees protocolFees
```
...
```solidity
contract ICandlV3LmPool lmPool
```
...
_Prevents calling a function from anyone except the address returned by ICandlV3Factory#owner()_
...
_The caller of this method receives a callback in the form of ICandlV3SwapCallback#candlV3SwapCallback_
...
_The caller of this method receives a callback in the form of ICandlV3FlashCallback#candlV3FlashCallback
...
```solidity
function setLmPool(contract ICandlV3LmPool _lmPool) external
```
