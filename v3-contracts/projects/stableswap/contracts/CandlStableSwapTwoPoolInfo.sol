...
contract CandlStableSwapTwoPoolInfo {
    ...
    function calc_coins_amount(address _swap, uint256 _amount) external view returns (uint256[N_COINS] memory) {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            uint256 value = (ICandlStableSwap(_swap).balances(i) * _amount) / total_supply;
            amounts[i] = value;
        }
        return amounts;
    }
    ...
    function get_add_liquidity_mint_amount(address _swap, uint256[N_COINS] memory amounts)
        ...
        returns (uint256)
    {
        ICandlStableSwap swap = ICandlStableSwap(_swap);
    ...
    function _xp(address _swap) internal view returns (uint256[N_COINS] memory result) {
        result = RATES(_swap);
        for (uint256 i = 0; i < N_COINS; i++) {
            result[i] = (result[i] * ICandlStableSwap(_swap).balances(i)) / PRECISION;
        }
    }
