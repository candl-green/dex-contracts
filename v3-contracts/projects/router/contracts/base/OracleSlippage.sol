...
import '@candl/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@candl/v3-periphery/contracts/libraries/Path.sol';
import '@candl/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@candl/v3-core/contracts/interfaces/ICandlV3Pool.sol';
import '@candl/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ICandlV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ICandlV3Pool pool) {
        pool = ICandlV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ICandlV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ICandlV3Pool(pool).slot0();
            }
