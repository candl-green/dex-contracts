...
describe('TickLens', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { factory, tokens, nft, deployer } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ factory, tokens, nft, deployer } = await loadFixture(nftFixture))
  })
  ...
  describe('#getPopulatedTicksInWord', () => {
    ...
    beforeEach(async () => {
      ...
      poolAddress = computePoolAddress(deployer.address, [tokens[0].address, tokens[1].address], FeeAmount.MEDIUM)
    })
