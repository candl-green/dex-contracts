...
} from '@candl/v3-core/artifacts/contracts/CandlV3PoolDeployer.sol/CandlV3PoolDeployer.json'
import { abi as FACTORY_V2_ABI, bytecode as FACTORY_V2_BYTECODE } from '@uniswap/v2-core/build/UniswapV2Factory.json'
import { Fixture, deployContract } from 'ethereum-waffle'
import { ethers, waffle } from 'hardhat'
import { ICandlV3Factory, IWETH9, MockTimeSwapRouter } from '../../typechain-types'
...
const wethFixture: Fixture<{ weth9: IWETH9 }> = async ([wallet]) => {
  const weth9 = (await deployContract(wallet, {
...
const v3CoreFactoryFixture: Fixture<{ deployer: Contract; factory: ICandlV3Factory }> = async ([wallet]) => {
  const deployer = await deployContract(wallet, {
