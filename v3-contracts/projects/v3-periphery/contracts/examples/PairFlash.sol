...
import '@candl/v3-core/contracts/interfaces/callback/ICandlV3FlashCallback.sol';
...
/// @notice An example contract using the CandlSwap V3 flash function
contract PairFlash is ICandlV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `CandlV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ICandlV3Pool pool = ICandlV3Pool(PoolAddress.computeAddress(deployer, poolKey));
