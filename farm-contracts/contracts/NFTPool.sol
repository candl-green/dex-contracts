 ...
 * esNFTs are yield-generating positions when the NFTPool contract has allocations from the Candl Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Candl staking position NFT", "fsNFT") {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated xCandl from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated xCandl boostPoints multiplier
    uint256 pendingxCandlRewards; // Not harvested xCandl rewards
    uint256 pendingCandlRewards; // Not harvested Candl rewards
  }
  ...
  ICandlMaster public master; // Address of the master
  ...
  IERC20 private _candlToken; // CandlToken contract's address
  IxCandlToken private _xCandlToken; // xCandlToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from xCandl yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for xCandlRewardsShare
  uint256 public xCandlRewardsShare = 8000; // 80%, directly defines candlShare with the remaining value to 100%
  ...
  function initialize(ICandlMaster master_, IERC20 candlToken, IxCandlToken xCandlToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _candlToken = candlToken;
    _xCandlToken = xCandlToken;
    initialized = true;

    // to convert CANDL to xCandl
   _candlToken.approve(address(_xCandlToken), type(uint256).max);
  }
  ...
  event SetxCandlRewardsShare(uint256 xCandlRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address candlToken, address xCandlToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_candlToken), address(_xCandlToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingxCandlRewards).add(position.pendingCandlRewards);
  }
  ...
  /**
   * @dev Set the share of xCandl for the distributed rewards
   * The share of CANDL will incidently be 100% - xCandlRewardsShare
   *
   * Must only be called by the owner
   */
  function setxCandlRewardsShare(uint256 xCandlRewardsShare_) external {
    _requireOnlyOwner();
    require(xCandlRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    xCandlRewardsShare = xCandlRewardsShare_;
    emit SetxCandlRewardsShare(xCandlRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external override nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingCandlRewards: 0,
      pendingxCandlRewards: 0
    });
   ...
   * Users should still be able to deallocate xCandl from the YieldBooster contract
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingxCandlRewards > 0 || position.pendingCandlRewards > 0) {
      uint256 xCandlRewards = pending.mul(xCandlRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 candlAmount = pending.add(position.pendingCandlRewards).sub(xCandlRewards);

      xCandlRewards = xCandlRewards.add(position.pendingxCandlRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingxCandlRewards = xCandlRewards;
        position.pendingCandlRewards = candlAmount;
      }
      else {
        // convert and send xCandl + CANDL rewards
        position.pendingxCandlRewards = 0;
        position.pendingCandlRewards = 0;

        if(xCandlRewards > 0) xCandlRewards = _safeConvertTo(to, xCandlRewards);
        // send share of CANDL rewards
        candlAmount= _safeRewardsTransfer(to, candlAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, candlAmount, xCandlRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _candlToken.balanceOf(address(this));
    // cap to available balance
    if (amount > balance) {
      amount = balance;
    }
    _candlToken.safeTransfer(to, amount);
    return amount;
  }

  /**
   * @dev Safe convert CANDL to xCandl function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _xCandlToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 candlAmount, uint256 xCandlAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, candlAmount, xCandlAmount), "non implemented");
