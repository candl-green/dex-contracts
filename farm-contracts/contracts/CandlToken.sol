...
/*
 * CANDL is Candl's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract CandlToken is Ownable, ERC20("Candl Exchange Token", "CANDL"), ICandlToken {
   ...
   */
  modifier onlyMaster() {
    require(msg.sender == masterAddress, "CandlToken: caller is not the master");
    _;
  }
  ...
  /**
   * @dev Burns "amount" of CANDL by sending it to BURN_ADDRESS
   */
  ...
  /**
   * @dev Updates CANDL emission rate per second
   *
   * Must only be called by the owner
   */
