...
/*
 * This contract is used to distribute dividends to users that allocated xCandl here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xCandl allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IxCandlTokenUsage, IDividends {
  ...
  address public immutable xCandlToken; // xCandlToken contract

  mapping(address => uint256) public usersAllocation; // User's xCandl allocation
  ...
  constructor(address xCandlToken_, uint256 startTime_) {
    require(xCandlToken_ != address(0), "zero address");
  ...
  modifier xCandlTokenOnly() {
    require(msg.sender == xCandlToken, "xCandlTokenOnly: caller should be xCandlToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xCandl to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xCandl allocation from this dividends contract
   *
   * Can only be called by xCandlToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxCandlTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xCandl is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userxCandlAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userxCandlAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userxCandlAllocation.mul(accDividendsPerShare).div(1e18);
