...
/*
 * This contract centralizes Candl's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the CANDLToken contract.
 */
contract CandlMaster is Ownable, ICandlMaster {
  ...
  ICandlToken private immutable _candlToken; // Address of the CANDL token contract
  ...
  constructor(
    ICandlToken candlToken_,
    uint256 startTime_
  ) {
    require(address(candlToken_) != address(0), "CandlMaster: candlToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= candlToken_.lastEmissionTime(), "CandlMaster: invalid startTime");
    _candlToken = candlToken_;
    startTime = startTime_; // Must be set with the same time as CandlToken emission start
  }
  ...
  /*
   * @dev Returns CandlToken address
   */
  function candlToken() external view override returns (address) {
    return address(_candlToken);
  }

  /*
   * @dev Returns CandlToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _candlToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(ICandlMaster, Ownable) returns (address) {
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 candlBalance = _candlToken.balanceOf(address(this));

    if (amount > candlBalance) {
      amount = candlBalance;
    }
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much CANDL rewards are expected to be received for this pool
      ...
      (rewards) = _candlToken.claimMasterRewards(rewards);
