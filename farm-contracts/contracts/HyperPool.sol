...
contract HyperPool is ReentrancyGuard, Ownable, INFTHandler {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    using SafeERC20 for IxCandlToken;
    ...
    struct Settings {
        ...
        string description; // Project's description for this HyperPool
    }
    ...
    IHyperPoolFactory public factory; // HyperPoolFactory address
    ICandlToken public candlToken; // CANDLToken contract
    IxCandlToken public xCandlToken; // xCandlToken contract
    INFTPool public nftPool; // NFTPool contract

    uint256 public creationTime; // Creation time of this HyperPool

    bool public published; // Is HyperPool published
    uint256 public publishTime; // Time at which the HyperPool was published
    ...
    constructor(
        ICandlToken candlToken_, IxCandlToken xCandlToken_, address owner_, INFTPool nftPool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(candlToken_) != address(0) && address(xCandlToken_) != address(0) && owner_ != address(0)
        ...
        factory = IHyperPoolFactory(msg.sender);
    ...
    /**
     * @dev Withdraw a position from the HyperPool
     *
    ...
    /**
     * @dev Withdraw a position from the HyperPool without caring about rewards, EMERGENCY ONLY
     *
    ...
    /**
     * @dev Harvest pending HyperPool rewards
     */
     ...
     * "to" can be set to this address only if this contract is allowed to transfer xCandl
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 candlAmount, uint256 xCandlAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        address owner = tokenIdOwner[tokenId];
        require(operator == owner, "not allowed");

        // if not whitelisted, the HyperPool can't transfer any xCandl rewards
        require(to != address(this) || xCandlToken.isTransferWhitelisted(address(this)), "cant handle rewards");

        // redirect rewards to position's previous owner
        if (to == address(this)) {
            candlToken.safeTransfer(owner, candlAmount);
            xCandlToken.safeTransfer(owner, xCandlAmount);
        }
    ...
    /**
     * @dev Transfer ownership of this HyperPool
     *
     * Must only be called by the owner of this contract
     */
    function transferOwnership(address newOwner) public override onlyOwner {
        _setHyperPoolOwner(newOwner);
    ...
    function renounceOwnership() public override onlyOwner {
        _setHyperPoolOwner(address(0));
        Ownable.renounceOwnership();
    }

    /**
     * @dev Add rewards to this HyperPool
     */
    function addRewards(uint256 amountToken1, uint256 amountToken2) external nonReentrant {
        ...
        // get active fee share for this HyperPool
        uint256 feeShare = factory.getHyperPoolFee(address(this), owner());
    ...
    /**
     * @dev Withdraw rewards from this HyperPool
     *
     * Must only be called by the owner
     * Must only be called before the publication of the Hyper Pool
     */
    ...
    /**
     * @dev Set requirements that positions must meet to be staked on this Hyper Pool
     *
     * Must only be called by the owner
     */
     ...
     *
     * Must only be called by the owner
     * Hyper duration can only be extended once already published
    ...
    /**
     * @dev Publish the Hyper Pool
     *
     * Must only be called by the owner
     */
    function publish() external onlyOwner {
        require(!published, "published");
        // this hyperPool is Stale
        ...
        factory.publishHyperPool(address(nftPool));
    ...
    /**
     * @dev Updates rewards states of this Hyper Pool to be up-to-date
     */
    ...
    /**
     * @dev Add a user's deposited amount into this Hyper Pool
     */
    ...
    /**
     * @dev Check whether a position with "tokenId" ID is meeting all of this Hyper Pool's active requirements
     */
  ...
  */
    ...
    function _setHyperPoolOwner(address newOwner) internal {
        factory.setHyperPoolOwner(owner(), newOwner);
    }
