...
import {ICandlPair} from "./interfaces/ICandlPair.sol";
import {ICandlRouter02} from "./interfaces/ICandlRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by CandlSwap teams.
 */
contract CandlZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // CandlRouter interface
    ICandlRouter02 public candlRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from CandlSwap)
    ...
    // Address CandlRouter
    address private candlRouterAddress;
     ...
     * @param _candlRouter: address of the CandlRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WETHAddress,
        address _candlRouter,
        uint256 _maxZapReverseRatio
    ) {
        WETHAddress = _WETHAddress;
        WETH = IWETH(_WETHAddress);
        candlRouterAddress = _candlRouter;
        candlRouter = ICandlRouter02(_candlRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. CANDL/ETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. CANDL) to receive in the intermediary swap (e.g. ETH --> CANDL)
     */
     ...
     * @param _lpToken: LP token address (e.g. CANDL/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. CANDL) in the intermediary swap (e.g. BUSD --> CANDL)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. CANDL/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. CANDL --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. CANDL/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. CANDL or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. CANDL) in the intermediary swap (e.g. BUSD --> CANDL)
     */
    ...
    {
        address token0 = ICandlPair(_lpToken).token0();
        address token1 = ICandlPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ICandlPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = candlRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = candlRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ICandlPair(_lpToken).token0() || _token0ToZap == ICandlPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ICandlPair(_lpToken).token0() || _token1ToZap == ICandlPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ICandlPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ICandlPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ICandlPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = candlRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = candlRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ICandlPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "CandlRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "CandlRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        candlRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), candlRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(candlRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = candlRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = candlRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = candlRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = candlRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = candlRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
