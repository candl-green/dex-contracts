...
import './CandlPair.sol';

contract CandlFactory is ICandlFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(CandlPair).creationCode));
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "CandlFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Candl: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Candl: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Candl: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(CandlPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ICandlPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "CandlFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "CandlFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, "CandlFactory: ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, "CandlFactory: ownerFeeShare mustn't exceed maximum");
